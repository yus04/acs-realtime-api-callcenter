...
print_debug("ACS_CONNECTION_STRING:", ACS_CONNECTION_STRING)
print_debug("CALLBACK_URI_HOST:", CALLBACK_URI_HOST)
print_debug("CALLBACK_EVENTS_URI:", CALLBACK_EVENTS_URI)
print_debug("AZURE_OPENAI_SERVICE_ENDPOINT:", AZURE_OPENAI_SERVICE_ENDPOINT)
print_debug("AZURE_OPENAI_SERVICE_KEY:", AZURE_OPENAI_SERVICE_KEY)
print_debug("AZURE_OPENAI_DEPLOYMENT_NAME:", AZURE_OPENAI_DEPLOYMENT_NAME)
...
async def handle_callback(contextId: str, request: Request):
    events = await request.json()
    print_debug("Callback events:", events)
    for event_dict in events:
        event = CloudEvent.from_dict(event_dict)
        print_debug("Callback event:", event)
...
async def receive_messages(call_id: str):  
    try:  
        ...
        while not gpt_client.closed:  
            ...
            if message:  
                print_debug(f"Received message from AI for call_id {call_id}: {message}")  
                if message.type == "response.audio.delta":  
                    ...
                    print_debug(f"Received transcript delta for call_id {call_id}: {transcript_delta}")    
    ...
    while True:
        try:
            ...
            if worker is not None:
                ...
                print_debug("Worker not found or not initialized.")
        except Exception as e:
